...
contract Assembly {
    ...
    function returnValueSOL() external view returns(uint) {
        return value;
    }
    function setNewValueSOL(uint number) external {
        value = number;
    }
    function addNumberSOL(uint _number) external {
        numbers[msg.sender] = _number;
    }
    function addNumbersListSOL(address[] memory _users, uint[] memory _numbers) external {
    ...
    function returnValueASS() external view returns(uint) {
    ...
    function setNewValueASS(uint _number) external {
        assembly {
            /// @dev variable.slot gives value of variable
            let slot := value.slot
            sstore(slot, _number)
    ...
    function addNumber(uint _numberAdded) external {
        assembly {
    ...
    function hashStringASS(string memory _string) external pure returns(bytes32) {
    assembly {
...
function addMultipleGuesses(address[] memory _users, uint[] memory _numbers) external {
        assembly {
            ...
            if iszero(eq(usersSize, numbersSize)) { revert(0, 0) }
